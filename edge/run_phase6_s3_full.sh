...
import json
from pathlib import Path
obj = json.loads(Path("$S3A_DIR/original_snapshot.json").read_text())
print(obj["machine_id"])
PY
)
...
import json
from pathlib import Path
obj = json.loads(Path("$S3C_DIR/snapshot.json").read_text())
print(obj["machine_id"])
PY
)
...
stderr_text = Path("$S3C_DIR/fabric_invoke.txt").read_text(encoding="utf-8", errors="ignore")
unauthorized_rejected = "$S3C_INVOKE_OK".lower() != "true"
...
for item in history:
    ...
    if not cid or not stored_hash:
        ...
        records.append({"cid": cid, "stored_hash": stored_hash, "ipfs_hash": None, "consistent": False})
        continue
    with urllib.request.urlopen("$IPFS_API/cat?arg=" + cid) as resp:
        data = resp.read()
