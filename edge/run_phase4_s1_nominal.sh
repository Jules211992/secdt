...
RUNSET_ID="phase4_s1_$(date -u +%Y%m%dT%H%M%SZ)"
...
for i in $(seq 1 "$N_RUNS"); do
  ...
  IDX=$(printf "%03d" "$i")
  MACHINE_ID="machine-s1-${RUN_TAG}-${IDX}"
  SESSION_ID="session-s1-${RUN_TAG}-${IDX}"
  TS=$(date -u +%Y-%m-%dT%H:%M:%SZ)
  CYCLE=$((200 + i))
  HEALTH_SCORE="94.0"

  cat <<JSON > "$RUN_DIR/snapshot.json"
{
...
with open(runset_dir / "runs_index.jsonl", "r") as f:
